...
public class LeaderboardShowcase : MonoBehaviour
{
    ...
    [SerializeField] private TextMeshProUGUI _playerUsernameInput;
    ...
    {
        _playerScoreText.text = $"Your Highscore: {_playerScore}";
    }
    ...
    private void Start()
    {
        _playerScore = (int)GameManager.instance.LowestTime;
        AddPlayerScore();
        InitializeComponents();
        Load();
        if (GameManager.instance.LowestTime!= 10000)
        {
            Submit();
        }
    }
    ...
    public void Submit()
    {
        Leaderboards.DemoSceneLeaderboard.UploadNewEntry(GameManager.instance.Username, _playerScore, Callback, ErrorCallback);
    }
